...
p_match' placer render style isInfix strictness m_pats GRHSs {..} = do
  ...
  let -- Calculate position of end of patterns. This is useful when we decide
      ...
      p_body = do
        ...
        sep breakpoint (located' (p_grhs' placer render groupStyle) . reLocA) grhssGRHSs
      p_where = do
        -- TODO valid refactoring?
...
p_grhs' ::
  ...
  R ()
p_grhs' placer render style (GRHS _ guards body) =
  case guards of
    [] -> p_body
    xs -> do
      txt "|"
    ...
    let isSection = any isMissing args
        ...
        parens' =
          case boxity of
            Boxed -> parens
            Unboxed -> parensHash
    -- TODO why distinction between section and non-section?
...
p_pat = \case
  ...
  ConPat _ pat details ->
    case details of
      -- TODO can the first field be ignored?
      PrefixCon _ xs -> sitcc $ do
        p_rdrName pat
        unless (null xs) $ do
          breakpoint
